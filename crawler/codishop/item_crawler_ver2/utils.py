...
def get_serial_number(product_info: List[WebElement]) -> str:
...
def get_season(product_info : List[WebElement]) -> Optional[str]:
    try:
...
def get_four_season(driver : webdriver.Chrome) -> Optional[str]:
    guide_all = driver.find_elements(By.CSS_SELECTOR, value="table.table-simple tr")
    four_season = list()
    for guide in guide_all :
        t = guide.find_element(By.CSS_SELECTOR, value="th").text
